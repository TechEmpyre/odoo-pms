    ...
    def property_to_sql(self, field_sql: SQL, property_name: str, model: BaseModel, alias: str, query: Query) -> SQL:
        ...
        if self.type == 'datetime' and (timezone := model.env.context.get('tz')):
            ...
            if timezone in pytz.all_timezones_set:
                # pytz still lists legacy names (e.g. Asia/Calcutta) that PostgreSQL may reject;
                # align with odoo._monkeypatches.pytz._tz_mapping for SQL timezone().
                from odoo._monkeypatches.pytz import _tz_mapping

                pg_tz = timezone
                while pg_tz in _tz_mapping:
                    pg_tz = _tz_mapping[pg_tz]
                sql_expr = SQL("timezone(%s, timezone('UTC', %s))", pg_tz, sql_expr)
            else:
