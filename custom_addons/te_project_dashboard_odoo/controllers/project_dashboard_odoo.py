...
#    TechEmpyre
#
#    Copyright (C) 2025-TODAY TechEmpyre(<https://www.techempyre.com>).
#    Author: AYANA KP @techempyre(contact@techempyre.com)
#
...
class ProjectFilter(http.Controller):
    ...
    @http.route('/project/task/count', auth='public', type='jsonrpc')
    ...
    @http.route('/employee/timesheet', auth='public', type='jsonrpc')
    ...
    @http.route('/project/filter', auth='public', type='jsonrpc')
    ...
    @http.route('/project/filter-apply', auth='public', type='jsonrpc')
    ...
    @http.route('/get/tiles/data', auth='public', type='jsonrpc')
    ...
    @http.route('/get/hours', auth='public', type='jsonrpc')
    ...
    @http.route('/get/task/data', auth='public', type='jsonrpc')
    def get_task_data(self):
        ...
        if request.env.user.has_group('project.group_project_manager'):
            request.env.cr.execute('''
                ...
                INNER JOIN project_project AS pro 
                    ON task.project_id = pro.id
                LEFT JOIN project_task_type AS stage 
            ...
            all_tasks = request.env['project.task'].search(
                [('project_id', 'in', all_project)])
            task_project = [[task.name, task.project_id.name, task.id, task.stage_id] for task
                            in
                            all_tasks]
            return {
