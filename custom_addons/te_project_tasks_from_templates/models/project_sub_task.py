...
class ProjectSubTask(models.Model):
    ...
    show_tasks_page = fields.Boolean(
        compute='_compute_show_tasks_page', readonly=False, string="Show Tasks Page",
        help='Indicates whether the Sub-Tasks page should be displayed.')
    state = fields.Selection([
        ...
        *CLOSED_STATES.items(),
        ...
        help='Represents the current status of the sub-task.')
    ...
    def _compute_show_tasks_page(self):
        ...
        record based on whether project_template_id present or not.
        If project_template_id present, 'show_tasks_page' is set to True,
        allowing the display of Sub-Tasks. If project_template_id
        are not present, 'show_tasks_page' is set to False."""
        for task in self:
