...
class ResConfigSettings(models.TransientModel):
    """
    Extends 'res.config.settings' to manage global configuration for project
    categories, document expiry notifications, and task overdue alerts.
    """
    ...
    def _onchange_is_project_category(self):
        """
        Dynamically updates the user's group membership for project categories
        ...
        if self.is_project_category:
            ...
            groups.write({'user_ids': [(3, self.env.user.id)]})
